...
1. ~~**批量配图**~~ ✅ 已完成（2026-08-17）：`make_figures.py` 扩至 6 种图型。⚠️ **2026-08-25 起已废弃**：起草哲学转为"召回+复用为主"，图片内容以历史真实图召回为主，make_figures.py 已删除，不再生成示意图（无召回图宁可不配图）。
...
│   └── make_figures.py            # ⚠️ 已删除（2026-08-25）：图片以召回为主，不再生成示意图
